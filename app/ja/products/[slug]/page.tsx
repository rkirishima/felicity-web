...
export async function generateMetadata({
  ...
  const description = `${product.country}産 ${product.process}精製 ${product.roast}ロースト — ${product.weight} ${product.price}`;
  ...
  return {
    ...
    alternates: {
      canonical: `https://felicity.cafe/ja/products/${slug}`,
      languages: {
    ...
    openGraph: {
      title,
      description,
      url: `https://felicity.cafe/ja/products/${slug}`,
      locale: "ja_JP",
      type: "website",
      images: [
        {
...
export default async function ProductPageJA({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: "ホーム", url: "https://felicity.cafe/ja/" },
    { name: "コーヒー豆", url: "https://felicity.cafe/ja/#coffee" },
    { name: product.name, url: `https://felicity.cafe/ja/products/${slug}` },
  ]);
    ...
    <main className="min-h-screen bg-[#F4EFE4]">
      ...
      <header className="border-b border-[#DDD5C5] px-8 py-5">
        <div className="max-w-6xl mx-auto flex justify-between items-baseline">
          ...
          <LanguageToggle currentLocale="ja" currentPath={`/ja/products/${slug}`} />
        </div>
      </header>
