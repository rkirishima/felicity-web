...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://felicity.cafe/products/${slug}`,
      languages: {
        ja: `https://felicity.cafe/products/${slug}`,
    ...
    openGraph: {
      title,
      description,
      url: `https://felicity.cafe/products/${slug}`,
      locale: "ja_JP",
      type: "website",
      images: [
        {
...
export default async function ProductPage({
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: "コーヒー豆", url: "https://felicity.cafe#coffee" },
    { name: product.name, url: `https://felicity.cafe/products/${slug}` },
  ]);
    ...
    <main className="min-h-screen bg-[#F4EFE4]">
      ...
      <header className="border-b border-[#DDD5C5] px-8 py-5">
        <div className="max-w-6xl mx-auto flex justify-between items-baseline">
          ...
          <LanguageToggle currentLocale="ja" currentPath={`/products/${slug}`} />
        </div>
      </header>
